...
def get_comments(video_id, max_page):
    ...
    while video_response:
        for item in video_response['items']:
            ...
            collected_comments.append(comment_text)
        if counter < max_page:
...
if __name__ == "__main__":
    ...
    req_c = 0
    failed = 0
    for vid_id in tqdm(df['id.videoId'].to_list(), leave=True):
        req_c += 1
        try:
            comments = get_comments(vid_id, 5)  # 5 requests per video id, 100 comments per request
            for comment in comments:
                comment_id['comment'].append(comment)
                comment_id['video_id'].append(vid_id)
        except Exception as e:
            print(vid_id, e)
            logging.error(msg=str(vid_id) + " : " + str(e) + '\n')
            failed += 1
